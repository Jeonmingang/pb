...
public class PocketPhotoCommand implements CommandExecutor {
    ...
    public PocketPhotoCommand(Main plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!plugin.isPixelmonPresent()) {
            p.sendMessage(ChatColor.RED + "Pixelmon 모드가 필요합니다.");
            return true;
        }
        if (args.length != 1) {
            p.sendMessage(ChatColor.YELLOW + "/포켓사진 <1~6>");
            return true;
        }
        int slot;
        try { slot = Integer.parseInt(args[0]); } catch (NumberFormatException e) {
            p.sendMessage(ChatColor.RED + "숫자 슬롯만 입력하세요 (1~6).");
            return true;
        }
        if (slot < 1 || slot > 6) {
        ...
        if (!res.success) {
            p.sendMessage(ChatColor.RED + res.message);
            return true;
        }

        boolean added = PhotoItemUtil.givePhotoItem(p, res);
        if (!added) {
            p.getWorld().dropItemNaturally(p.getLocation(), PhotoItemUtil.buildPhotoItem(p, res));
            p.sendMessage(ChatColor.GRAY + "인벤토리가 가득 차 사진 아이템을 바닥에 떨어뜨렸습니다.");
        }
