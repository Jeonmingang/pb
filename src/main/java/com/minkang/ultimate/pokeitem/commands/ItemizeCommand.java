    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (cmd.getName().equals("포켓몬아이템화")) {
            ...
            try {
                ...
                p.sendMessage(plugin.msg("invalid_slot"));
                return true;
            }
            if (slot < 1 || slot > 6) {
                p.sendMessage(plugin.msg("invalid_slot"));
            ...
            if (!hook.isPixelmonPresent()) {
                p.sendMessage(plugin.msg("no_pixelmon"));
                return true;
            }

            PokemonInfo info = hook.readPokemon(p, slot - 1);
            if (info == null) {
                p.sendMessage(plugin.msg("empty_slot"));
                return true;
            }
            ...
            // Remove original (safe: only after item creation)
